...
#define GPIO_CLOCK_ENREG        (0x40023800 + 0x30)                      /* GPIO clock enable register */
...
static void stm32_gpio_clk_enable(stm32_gpio_dsc_t * dsc)
{
    int reg;

    reg = readl(GPIO_CLOCK_ENREG);
    reg |= (1 << dsc->port);
    writel(GPIO_CLOCK_ENREG, reg);
}

static void stm32_gpio_output(stm32_gpio_dsc_t * dsc, int val)
{
