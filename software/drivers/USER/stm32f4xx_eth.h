...
#include "el_nlist.h"
struct stm32_eth {
	struct list_head nbuf_head;
	sys_mutex_t lock;
    sys_sem_t sem;		/* recieve sync */
};
/* DMA Descriptors. */
typedef struct
{
...
typedef struct
{
    unsigned int volatile ctrlstat;
    unsigned int size;
    unsigned int addr;
    unsigned int next;
} stm32_tx_desc_t;
...
extern void eth_ll_frame_transmit(char * data, int len);
extern int stm32_eth_ll_init_rmii(void);
#endif
